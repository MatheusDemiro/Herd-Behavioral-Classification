...
class PreProcessing:
    def __init__(self):
        self._path = "C:\Temp\Herd-Behavioral-Classification\dataset\data.csv"
    ...
    def clearData(self, estimator):
        ...
        for i in range(len(self._data)):
            ...
            self._data.loc[i, 'tpo-pasto'] = int(attr[i][4]) / 1000
            self._data.loc[i, 'tbs-sombra'] = int(attr[i][5]) / 1000
            self._data.loc[i, 'ur-sombra'] = int(attr[i][6]) / 1000
            self._data.loc[i, 'tpo-sombra'] = int(attr[i][7]) / 1000
